...
CVSID("$Id: job.c,v 1.1 2001-11-13 03:02:55 gnb Exp $");
...
{
    if (job->ops != 0 && job->ops->delete != 0)
    	(*job->ops->delete)(job->userdata);
...
main_thread(void)
{
	...
	while ((job = queue_tryget(finish_queue)) != 0)
	...
	else
	{
	    ...
	    finish_job(queue_get(finish_queue));
	}
    }
