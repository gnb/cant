...
CVSID("$Id: xml.c,v 1.6 2001-11-18 11:17:01 gnb Exp $");
...
node_info_insert(xmlNode *node, const char *filename, int lineno)
{
    ...
    if ((savedfn = g_hash_table_lookup(filenames, filename)) == 0)
    {
...
{
    return (node == 0 ? 0 : g_hash_table_lookup(node_infos, (gpointer)node));
}
